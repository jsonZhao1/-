...
export default {
  ...
  data() {
    return {
      ...
      examStatus: 0,
      continueStatus: 0,
      question: '请问以下场景将用到哪几个标识？请点击右侧选项确定答案',
      checkList: [],
      dialogVisible: false,
  ...
  computed: {
    residue() {
      return Number(this.form.classList.length) - Number(this.checkList.length)
    },
    myChose() {
      return this.selectorDataList.filter(item => this.checkList.includes(item.id))
    },
    correctAnswer() {
      return this.selectorDataList.filter(item => this.form.classList.includes(item.id + ''))
    }
  },
  methods: {
    closeDialogVisible() {
    ...
    startExam() {
      ...
      const matchIds = new Set(this.form.classList)
      let query = this.db.markStudy.toCollection().filter(record => record.className.includes(this.queryParams.className))
      query.toArray().then(res => {
        // 从原始数组中筛选出包含特定选项的项
        const itemsWithSpecificItems = res.filter(item => this.form.classList.includes(item.id))

        // 从原始数组中去除包含特定选项的项
        const remainingItems = res.filter(item => !this.form.classList.includes(item.id))

        // 计算需要从包含特定选项的项和剩余的项中各自选择的数量
        const specificItemsCount = Math.min(24, itemsWithSpecificItems.length)
        const remainingItemsCount = Math.min(24 - specificItemsCount, remainingItems.length)

        // 随机选择包含特定选项的项和剩余的项
        const randomItemsWithSpecificItems = this.getRandomItems(itemsWithSpecificItems, specificItemsCount)
        const randomRemainingItems = this.getRandomItems(remainingItems, remainingItemsCount)

        // 合并两部分随机选择的结果
        const resultArray = randomItemsWithSpecificItems.concat(randomRemainingItems)
        this.selectorDataList = resultArray
    ...
    getRandomItems(array, count) {
      const randomItems = []
      const shuffledArray = array.slice() // 创建一个数组副本，以免修改原始数组

      while (randomItems.length < count && shuffledArray.length > 0) {
        const randomIndex = Math.floor(Math.random() * shuffledArray.length)
        randomItems.push(shuffledArray.splice(randomIndex, 1)[0])
      }
    ...
    confirmExam() {
      ...
      if (this.checkList.length > 0) {
        ...
        for (let i = 0; i < this.checkList.length; i++) {
          // 检查当前元素是否在array2中存在
          if (this.form.classList.includes(this.checkList[i] + '')) {
            // 如果存在，则增加计数器的值
            count++
          }
        }
        console.log(count)
        this.oneScore = (10 / this.form.classList.length) * count
        console.log(this.oneScore)
        let message = this.form
        message.total = this.total
        message.totalCn = this.totalCn
        message.type = 2
        message.questionType = 2
        message.selectorDataList = this.selectorDataList
    ...
    <el-container class="exam-desc-container">
      ...
      <el-main>
          ...
          <el-form :model="form" label-width="80px">
            <div class="exam-desc-question">
              ...
              <div style="position: absolute; margin-left: 100px; margin-top: 128px; width: 650px">
                <span style="font-size: 30px; color: #FFFFFF">{{ total }}.请问以下场景将用到哪几个标识？请点击<span
                    style="color: #E6A23C"
                >右侧</span>选项确定答案</span>
              </div>
              <div style="position: absolute; margin-left: 160px; margin-top: 230px">
                <img :src="form.sceneUrl" style="width: 500px;height: 500px" alt="">
              </div>
            </div>
            <div v-if="continueStatus === 0" class="exam-desc-answer">
              <div>
                <div class="exam-desc-selector"
                     style="display: flex;align-items: center;flex-wrap: wrap;margin-top: 30px;padding: 0 80px;height: 565px"
                >
                  <div v-for="(item,index) in selectorDataList" :key="index" class="itemStyle"
                       @click="changeSelector(item,index)" :class="checkList.includes(item.id)?'itemBg2':'itemBg1'"
                  >
                    <img :src="item.identifierUrl" alt="" style="width: 85px;height: 85px">
                  </div>

                </div>
                <div style="margin-top: 60px;text-align: center">
                  <span style="font-size: 30px; color: #FFFFFF;">剩余未选择：<span>{{ residue }}</span></span>
                </div>
              </div>
              <div style="display: flex;justify-content: space-evenly;margin-top: 50px">
                <div class="exam-desc-selector-confirm-chexiao" @click="cleanList" style="margin: 0"></div>
                <div class="exam-desc-selector-confirm" @click="confirmExam" style="margin: 0"></div>
              </div>
            </div>
            <div v-else class="exam-desc-score">
              ...
              <div>
                <div class="exam-result-answer-true" style="display: flex;flex-wrap: wrap;">
                  <div v-for="(item,index) in correctAnswer" :key="index" class="itemBg2" style="display:flex;
                  width: 70px;height: 70px;align-items: center;justify-content: center;margin-right: 10px"
                  >
                    <img :src="item.identifierUrl" alt="" style="width: 55px;height: 55px">
                  </div>
                </div>
                <div class="exam-result-answer-your" style="display: flex;flex-wrap: wrap;">
                  <div v-for="(item,index) in myChose" :key="index" class="itemBg1" style="display:flex;
                  width: 70px;height: 70px;align-items: center;justify-content: center;margin-right: 10px"
                  >
                    <img :src="item.identifierUrl" alt="" style="width: 55px;height: 55px">
                  </div>
              ...
              <!--              继续按钮-->
              <div class="exam-desc-selector-continue" @click="continueExam(1,checkList[0],correctAnswer)"></div>
            </div>
          </el-form>
        </div>
        <!--   结算页面   -->
        <div v-if="examStatus === 2" class="exam-main">
          ...
          <div class="exam-rank-btn" @click="addRank(queryParams.classId, scoreStore)">
          </div>
        </div>
      </el-main>
    </el-container>
