...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        identifierPic: [{ required: true, message: '展示图片图片不能为空', trigger: 'change' }]
      },
      open: false,
      actionUrl: '',
      imgFileList: [],
      ...
      listHander: {
        ...
        identifierPic: '展示图片(直接贴图(请用浮动图片)，图片不能超出单元格)'
      },
      listHandler: {
        ...
        '展示图片(直接贴图(请用浮动图片)，图片不能超出单元格)': 'identifierPic'
      },
      imageList: [],
  ...
  methods: {
    ...
    handleDelete(row) {
      // 执行删除操作
      this.db.sceneData.toArray().then(res => {
        const hasBinding = res.some(item => item.classList && item.classList.includes(row.id + ''))
        if (hasBinding) {
          this.showMessage({
            message: '此标识绑定了场景考试，请先解绑在删除',
            type: 'error',
            duration: 1000
          })
        } else {
          this.db.markStudy.delete(row.id).then(res => {
            this.showMessage({
              message: '删除成功',
              type: 'success',
              duration: 1000
    ...
    DownloadHandleDelete() {
      this.db.sceneData.toArray().then(res => {
        const hasBinding = res.some(item => {
          if (!item.classList) return false
          return item.classList.some(classId => this.ids.includes(parseInt(classId)))
        })
        if (hasBinding) {
          this.showMessage({
            message: '有标识绑定了场景考试，请先解绑在删除',
            type: 'error',
            duration: 1000
          })
        } else {
          this.db.markStudy.bulkDelete(this.ids).then(res => {
            this.showMessage({
              message: '删除成功',
              type: 'success',
              duration: 1000
    ...
    handleUpdateFlag(row) {
      ...
      this.db.markStudy.update(row.id, row).then(res => {
        this.showMessage({
          message: '修改成功',
          type: 'success',
          duration: 1000
      ...
      this.$refs['identifierClassRef'].validate(valid => {
        if (valid) {
          ...
          if (this.form.id != undefined) {
            this.db.markStudy.update(this.form.id, this.form).then(res => {
              this.showMessage({
                message: '修改成功',
                type: 'success',
                duration: 1000
            ...
            this.db.markStudy.add(this.form).then(() => {
              this.showMessage({
                message: '新增成功',
                type: 'success',
                duration: 1000
    ...
    picfileChange(file, fileList) {
      console.log(file)
      this.picFileList = fileList.slice(-1)
      let path = file.raw.path
      console.log(path)
      this.form.identifierPic = path
    },
    cancel() {
      this.reset()
      this.open = false
    ...
    importTemplate() {
      DExcel([], this.listHandler, '标识学习导入')
    },
    readFile(file) {
    ...
    async getExcelImage(file) {
      ...
      try {
        ...
        console.log('ZIP文件内容:', Object.keys(zipLoadResult['files']))
        ...
        console.log('排序后的文件键:', sortedKeys)
        ...
        for (const key of sortedKeys) {
          ...
          // 现在可以按自然排序顺序处理每个键
          if ((fileName.indexOf('media/image') != -1 || fileName.indexOf('xl/media/image') != -1) && !zipLoadResult['files'][key].dir) {
            ...
            await zip
              ...
              .then((res) => {
                imageList.push(res)
                console.log('成功提取图片，当前图片数量:', imageList.length)
              })
          }
        }
      } catch (error) {
        console.log('提取图片时出错:', error)
      }
      console.log('最终提取的图片数量:', imageList.length)
      return imageList
    },
    submitFileForm() {
      this.db.markStudy.bulkPut(this.importDataList).then(() => {
        this.showMessage({
          message: '新增成功',
          type: 'success',
          duration: 1000
  ...
  <div style="height:calc(100vh - 80px) ">
    <el-row :gutter="24" style="height: 100%">
      ...
      <el-col :span="20" :xs="24">
        ...
        <el-table :data="list" @selection-change="handleSelectionChange" height="70vh">
          ...
          />
          <el-table-column label="标识图片" align="center" key="identifierUrl" prop="identifierUrl" width="180">
            <template #default="scope">
              <img :src="scope.row.identifierUrl" alt="" style="width: 100px;height: 100px">
            </template>
          </el-table-column>
          <el-table-column label="展示图片" align="center" key="identifierPic" prop="identifierPic" width="180">
            <template #default="scope">
              <img :src="scope.row.identifierPic" alt="" style="width: 100px;height: 100px">
            </template>
          </el-table-column>
      ...
      <el-form :model="form" :rules="rules" ref="identifierClassRef" label-width="120px">
        ...
        <el-form-item label="标识图片" prop="identifierUrl">
          <el-input style="width: 300px; display: inline-block;" :disabled="true"
                    v-model="form.identifierUrl" placeholder="请选择封面图片"
          ></el-input>
          <el-upload
        ...
        <el-form-item label="展示图片" prop="identifierPic">
          <el-input style="width: 300px; display: inline-block;" :disabled="true"
                    v-model="form.identifierPic" placeholder="请选择封面图片"
          ></el-input>
          <el-upload
            style="display: inline-block;"
            :action="actionUrl"
    ...
    <el-dialog :title="title" :visible.sync="disableFlag" width="700px" append-to-body>
      <el-form :model="form" label-width="120px" v-if="disableFlag">
        ...
        <el-form-item label="展示图片" prop="identifierPic">
          <img :src="form.identifierPic" alt="" style="width: 100px;height: 100px">
          <!--          <ImagePreview width="100px" height="100px" :src="form.identifierPic"></ImagePreview>-->
        </el-form-item>
      </el-form>
      <template #footer>
        <div class="dialog-footer">
