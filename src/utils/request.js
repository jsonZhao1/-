...
import { tansParams, blobValidate } from '@/utils/yyh'
...
service.interceptors.request.use(config => {
  ...
  if (!isRepeatSubmit && (config.method === 'post' || config.method === 'put')) {
    ...
    if (sessionObj === undefined || sessionObj === null || sessionObj === '') {
      ...
      console.log(requestObj.time - s_time)
...
service.interceptors.response.use(res => {
  console.log(res)
    ...
    if (code === 401) {
      if (!isRelogin.show) {
        if(!res.config.url.includes('devicePlatform/deviceHeartBeat')){
          isRelogin.show = true;
          MessageBox.confirm(
      ...
      return Promise.reject(new Error(msg))
    } else if (code !== 200 && code !== 201 && code !== 203) {
